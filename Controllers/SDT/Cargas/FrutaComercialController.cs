...
namespace ProvexApi.Controllers.SDT.Cargas
{
    ...
    [ApiController]
    [Route("api")]
    public class FrutaComercialController : ControllerBase
    {
        ...
        public FrutaComercialController(IHttpClientFactory httpClientFactory, IConfiguration configuration, ProvexDbContext db, TemporadaService temporadaService)
        {
        ...
        [HttpGet("SdtFrutaComercial")]
        public async Task<IActionResult> GetFrutaComercial(
        [FromQuery] bool guardarEnBD = true, [FromQuery] string? temporada = null, [FromQuery] string? empresa = null)
        {
            try
            {
                ...
                var api = SdtApiHelper.GetSdtApiConfig(_configuration, "InformeFrutaComercial");
                var url = api.Url.Replace("{temporada}", codigoInterno);
                var dataResponse = await SdtApiHelper.GetDataAsync<FrutaComercialResponse>(url, api.ApiAuthUser, api.ApiAuthPassword, api.ApiTimeOut);
            ...
            {
                LogHelper.Log("Fruta Comercial | Error : " + ex.Message, "Controller");
                return await ErrorResponseHelper.HandleExceptionAsync(ex, _configuration, "Fruta Comercial");
            }
        }
    }
}
