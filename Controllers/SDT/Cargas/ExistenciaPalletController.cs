...
namespace ProvexApi.Controllers.SDT.Cargas
{
    ...
    [ApiController]
    [Route("api")]
    public class ExistenciaPalletController : ControllerBase
    {
        ...
        public ExistenciaPalletController(IHttpClientFactory httpClientFactory, IConfiguration configuration, ProvexDbContext db, TemporadaService temporadaService)
        {
        ...
        [HttpGet("SdtExistenciaPallet")]
        public async Task<IActionResult> GetExistenciaPallet(
        [FromQuery] bool guardarEnBD = true, [FromQuery] string? temporada = null, [FromQuery] string? empresa = null)
        {
            try
            {
                ...
                var api = SdtApiHelper.GetSdtApiConfig(_configuration, "InformeExistenciaPallet");
                var url = api.Url.Replace("{temporada}", codigoInterno);
                var dataResponse = await SdtApiHelper.GetDataAsync<ExistenciaPalletResponse>(url, api.ApiAuthUser, api.ApiAuthPassword, api.ApiTimeOut);
            ...
            {
                LogHelper.Log("Existencia Pallet | Error : " + ex.Message, "Controller");
                return await ErrorResponseHelper.HandleExceptionAsync(ex, _configuration, "Existencia Pallet");
            }
        }
    }
}
