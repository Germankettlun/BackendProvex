...
namespace ProvexApi.Controllers.SDT.Cargas
{
    ...
    public class DespachoController : ControllerBase
    {
        ...
        public DespachoController(IHttpClientFactory httpClientFactory, IConfiguration configuration, ProvexDbContext db, TemporadaService temporadaService)
        {
        ...
        [FromQuery] bool guardarEnBD = true, [FromQuery] string? temporada = null, [FromQuery] string? empresa = null)
        {
            try
            {
                ...
                var api = SdtApiHelper.GetSdtApiConfig(_configuration, "InformeDespacho");
                var url = api.Url.Replace("{temporada}", codigoInterno);
                var dataResponse = await SdtApiHelper.GetDataAsync<DespachoResponse>(url, api.ApiAuthUser, api.ApiAuthPassword, api.ApiTimeOut);
            ...
            {
                LogHelper.Log("Despachos | Error : " + ex.Message, "Controller");
                return await ErrorResponseHelper.HandleExceptionAsync(ex, _configuration, "Despachos");
            }
        }
    }
}
