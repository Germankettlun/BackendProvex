...
namespace ProvexApi.Controllers.SDT.Cargas
{
    ...
    [ApiController]
    [Route("api")]
    public class RecepcionFrutaEmbaladaController : ControllerBase
    {
        ...
        public RecepcionFrutaEmbaladaController(IHttpClientFactory httpClientFactory, IConfiguration configuration, ProvexDbContext db, TemporadaService temporadaService)
        {
        ...
        [HttpGet("SdtRecepcionFrutaEmbalada")]
        public async Task<IActionResult> GetRecepcionFrutaEmbalada(
        [FromQuery] bool guardarEnBD = true, [FromQuery] string? temporada = null, [FromQuery] string? empresa = null)
        {
            try
            {
                ...
                var api = SdtApiHelper.GetSdtApiConfig(_configuration, "InformeRecepcionesFrutaEmbalada");
                var url = api.Url.Replace("{temporada}", codigoInterno);
                var dataResponse = await SdtApiHelper.GetDataAsync<RecepcionFrutaResponse>(url, api.ApiAuthUser, api.ApiAuthPassword, api.ApiTimeOut);
            ...
            {
                LogHelper.Log("Recepcion Fruta embalada | Error : " + ex.Message, "Controller");
                return await ErrorResponseHelper.HandleExceptionAsync(ex, _configuration, "Recepcion Fruta embalada");
            }
        }
    }
}