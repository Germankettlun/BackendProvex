...
namespace ProvexApi.Controllers.SDT.Cargas
{
    ...
    public class CuentasCorrientesController : ControllerBase
    {
        ...
        public CuentasCorrientesController(IHttpClientFactory httpClientFactory, IConfiguration configuration, ProvexDbContext db, TemporadaService temporadaService)
        {
        ...
        [HttpGet("SdtCuentasCorrientes")]
        public async Task<IActionResult> GetCuentasCorrientes(
        [FromQuery] bool guardarEnBD = true, [FromQuery] string? temporada = null, [FromQuery] string? empresa = null)
        {
            try
            {
                ...
                var api = SdtApiHelper.GetSdtApiConfig(_configuration, "InformeCuentasCorrientes");
                var url = api.Url.Replace("{temporada}", codigoInterno);
                var dataResponse = await SdtApiHelper.GetDataAsync<CuentasCorrientesResponse>(url, api.ApiAuthUser, api.ApiAuthPassword, api.ApiTimeOut);
            ...
            {
                LogHelper.Log("Cargos Manuales | Error : " + ex.Message, "Controller");
                return await ErrorResponseHelper.HandleExceptionAsync(ex, _configuration, "Cuentas Corrientes");
            }
        }

        [HttpGet("SdtCuentaCorrienteRecibidor")]
        public async Task<IActionResult> GetCuentaCorrienteRecibidor([FromQuery] bool guardarEnBD = true, [FromQuery] string? temporada = null, [FromQuery] string? empresa = null)
        {
            try
            {
                if (string.IsNullOrEmpty(temporada))
                ...
                var api = SdtApiHelper.GetSdtApiConfig(_configuration, "InformeCuentasCorrientesRecibidor");
                var url = api.Url.Replace("{temporada}", codigoInterno);       
                var dataResponse = await SdtApiHelper.GetDataAsync<CuentaCorrienteRecibidorResponse>(url, api.ApiAuthUser, api.ApiAuthPassword, api.ApiTimeOut);
                dataResponse.Data.ForEach(item => item.CodigoEmpresa = empresa);  //Cambiar dinamicamente por Json
                dataResponse.Data.ForEach(item => item.CodigoTemporada = temporada);
            ...
            {
                LogHelper.Log("Cuenta corriente Recibidor | Error : " + ex.Message, "Controller");
                return await ErrorResponseHelper.HandleExceptionAsync(ex, _configuration, "Cuenta corriente Recibidor");
            }
        }
    }
}
