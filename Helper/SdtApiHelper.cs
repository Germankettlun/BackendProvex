...
public static class SdtApiHelper
{
    public class SdtApiConfig
    {
        ...
        public string? ConBDExtranet { get; set; }  // Opcional - no todas las APIs la necesitan
    ...
    public static SdtApiConfig GetSdtApiConfig(IConfiguration configuration, string apiName)
    {
        try
        {
            // Conexion BD - Usar la misma configuración que el resto de la aplicación
            // ANTES: leía desde ConnectionProvexStrings:DatabaseConnection (podía estar hardcodeada al .251)
            // AHORA: lee desde ConnectionStrings:ProvexDb que es la que se configura dinámicamente según ambiente
            string? provexConn = configuration.GetConnectionString("ProvexDb");
            string? extranetConn = configuration.GetConnectionString("ExtranetDb");  // Opcional
            ...
            return new SdtApiConfig
            {
                ...
                ConBDExtranet = extranetConn,  // Puede ser null - no todas las APIs la necesitan
